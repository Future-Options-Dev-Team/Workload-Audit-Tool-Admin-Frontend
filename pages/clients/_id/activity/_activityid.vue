...
export default {
  ...
  components: { BaseModal, SurveyForm, SplitWorkloadForm, BaseBreadcrumb },

  async asyncData({ $axios, params }) {
    const [activity, client] = await Promise.all([
      $axios.$get(`activities/${params.activityid}`),
  ...
  data() {
    return {
      ...
      items: [
        ...
        { label: this.client ? this.client.name : `Client details`, name: 'single-client', to: `/clients/${this.$route.params.id}` },
        { label: `Activity`, name: 'tasks', to: `/clients/${this.$route.params.id}` },
      ],
    }
  },
