  ...
  <div>
    ...
    <div class="mt-2 border-t px-4">
      ...
      <div v-for="index in participants" :key="index" class="mt-5 py-3 px-4 border rounded">
        ...
        <v-text-field
          :value="newParticipants[index - 1].position_workload_driver"
          :error-messages="createForm(index - 1).errors.get('position_workload_driver')"
          outlined
          dense
          light
          label="Position workload driver"
          @input="storeInput($event, index, 'position_workload_driver')"
        ></v-text-field>
      </div>
...
export default {
  ...
  methods: {
    ...
    getTaskSurveys() {
      this.loading = true
      this.$axios.$get(`/task-workload-drivers/task/${this.task.id}`)
        .then((response) => {
    ...
    submit() {
      this.loading = true
      Promise.all(
        this.newParticipants.map(participant => this.$axios.$post(`/task-workload-drivers`, this.$form(participant)))
      ).then(() => {
        this.loading = false
        this.$toast.success('Workload driver saved successfully!')
        this.$nuxt.refresh()
        this.done()
      }).catch((error) => {
        this.loading = false
